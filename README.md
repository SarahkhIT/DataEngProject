...
- **Interactive querying** — the final cell prompts the user to type their own question at runtime, rather than running against a fixed example question.
...
9. **Test the RAG pipeline** by running the final cell — it will prompt you with `What is your question?`, so you can type in your own question interactively rather than using a hardcoded example. The generated answer, along with its `Sources` section, will print below.
